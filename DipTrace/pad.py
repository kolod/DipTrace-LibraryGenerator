...
from typing import Literal, AnyStr, List, Any, Union
from pyfields import field, init_fields

from DipTrace.reHelper import reBracketed, reJoin, reInt, reString, reFloat, searchDoubleFloat, searchSingleBool, searchSingleFloat, searchSingleInt
...
from DipTrace.enums import DipTraceHoleTypes, DipTracePadShapes, DipTracePadShapesNew
...
class DipTracePad:
    ...
    top_mask: List[Union[str, int]] = field(default=['N', 0, 0], doc='')
    bottom_mask: List[Union[str, int]] = field(default=['N', 0, 0], doc='')
    top_paste: List[Union[str, int]] = field(default=['N', 0, 0], doc='')
    bottom_paste: List[Union[str, int]] = field(default=['N', 0, 0], doc='')

    @standart.converter(accepts=bool)
    @locked.converter(accepts=bool)
    @surface.converter(accepts=bool)
    @inverted.converter(accepts=bool)
    @sided.converter(accepts=bool)
    ...
    def calculateOld(self) -> None:
        if (self.shape_new == DipTracePadShapesNew.Rectangle) and (self.pad_corner > 0):
            self.points = []
            self.shape = DipTracePadShapes.Polygon

            r = min(self.width, self.height) * \
                self.pad_corner / 100   # Corner radius
            x = [(self.width / 2 - r) + r * math.sin(math.radians(float(a)))
                 for a in range(0, 91, 15)]
            y = [(self.height / 2 - r) + r * math.sin(math.radians(float(a)))
                 for a in range(0, 91, 15)]

            for i in range(len(x)):
                self.points.append(DipTracePoint(x=x[i], y=y[-i-1]))
            for i in range(len(x)):
                self.points.append(DipTracePoint(x=x[-i-1], y=-y[i]))
            for i in range(len(x)):
                self.points.append(DipTracePoint(x=-x[i], y=-y[-i-1]))
            for i in range(len(x)):
                self.points.append(DipTracePoint(x=-x[-i-1], y=y[i]))
    ...
    def _mask(match: re.Match[Any]):
        ...
        if match.lastindex >= 3:
            result.append(int(match.group(3)))
        return result
            ...
            if line == ')':
                ...
                while line := datafile.readline().strip():
                    ...
                    if tmp := re.search(reBracketed(reJoin(r'pt', reFloat, reFloat, reFloat, reFloat)), line):
                        if tmp.lastindex >= 4:
                            self.mask_top_segments.append(
                                [tmp.group(i+1) for i in range(4)])
                ...
                while line := datafile.readline().strip():
                    ...
                    if tmp := re.search(reBracketed(reJoin(r'pt', reFloat, reFloat, reFloat, reFloat)), line):
                        if tmp.lastindex >= 4:
                            self.mask_bottom_segments.append(
                                [tmp.group(i+1) for i in range(4)])
    ...
    def __str__(self):
        ...
        top_segmens = '\n'.join(
            [f'(pt {s[0]:.5g} {s[1]:.5g} {s[2]:.5g} {s[3]:.5g}' for s in self.mask_top_segments])
        bottom_segmens = '\n'.join(
            [f'(pt {s[0]:.5g} {s[1]:.5g} {s[2]:.5g} {s[3]:.5g}' for s in self.mask_bottom_segments])
        ...
        if self.isComponent:
            return ''.join([
                ...
                f'(PadWidth {mm2units(self.width):.5g})\n',
                f'(PadHeight {mm2units(self.height):.5g})\n',
                f'(PadHole {mm2units(self.hole_width):.5g})\n',
                f'(PadHoleH {mm2units(self.hole_height):.5g})\n',
                ...
                f'(PadWidth_New {mm2units(self.width):.5g})\n',
                f'(PadHeight_New {mm2units(self.height):.5g})\n',
                f'(Group {self.group})\n',
            ...
            return ''.join([
                f'(Pad {"{0}"} "{self.name}" "{self.note}" {mm2units(self.x):.5g} {mm2units(self.y):.5g}\n',
                f'(Number {self.number})\n',
                f'(Number_New {self.number})\n',
                f'(Inverted "{self.inverted}")\n',
                f'(Locked "{self.locked}")\n',
                ...
                f'(Standard "{self.standart}")\n',
                f'(Points\n{points}\n)\n',
                f'(PadPoints_New\n{points_new}\n)\n' if self.standart and (len(self.points_new) or len(self.points)) else '',
                f'(PadTerminalCount {len(self.terminals)}\n{terminals}\n)\n',
                f'(PadMask_Percent {self.mask_percent:0.2g})\n',
                f'(PadMask_EdgeGap {self.mask_edge_gap:0.1g})\n',
                f'(PadMask_SegmentGap {self.mask_segment_gap:0.1g})\n',
                f'(PadMask_SegmentSide {self.mask_segment_side:0.4g})\n',
